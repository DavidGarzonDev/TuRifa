...
export const createRifas = async (req, res) => {
    try {
        ...
        const { data, error } = await createRifa(rifaData);
        if (error) {
            console.log("Error details:", error);
            return res.status(500).json({ error: "Error al crear la rifa" });
        }

        res.status(201).json({ rifa: data });
    } catch (error) {
...
export const decrementRifaTicket = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Tickets de la rifa decrementados correctamente", data });
...
export const checkRifaForDraw = async (req, res) => {
    try {
        ...
        const { data: readyData, error } = await rifaModel.isRifaReadyForDraw(rifaId);
        if (error) {
            return res.status(500).json({ error: "Error al verificar la rifa" });
        }

        return res.status(200).json({
            isReady: readyData.isReady,
            reason: !readyData.isReady ? readyData.reason : null,
            message: readyData.isReady ? "La rifa está lista para sorteo" : "La rifa no está lista para sorteo"
        });
    } catch (error) {
...
export const realizarSorteo = async (req, res) => {
    try {
        ...
        const { data: readyData, error: readyError } = await rifaModel.isRifaReadyForDraw(rifaId);
        if (readyError) {
            return res.status(500).json({ error: "Error al verificar rifa" });
        }

        if (!readyData.isReady) {
            return res.status(400).json({ error: readyData.reason || "Esta rifa no está lista para sorteo" });
        }

        const ticketsResult = await ticketModel.getTicketsByRifaId(rifaId);
        ...
        // Asegurar que el ticket tenga numeroBoleto
        if (!ganador.numeroBoleto) {
            ganador.numeroBoleto = `#${String(ganador.id).padStart(4, "0")}`;
        }

        await rifaModel.updateRifaStatus(rifaId, "Cerrada");
        await rifaModel.setRifaWinner(rifaId, ganador.idUser, ganador.id);

        const user = await getUserByUid(ganador.idUser);
        ...
        const ganadorInfo = {
            ticketId: ganador.id,
            ticketNumber: ganador.numeroBoleto,
