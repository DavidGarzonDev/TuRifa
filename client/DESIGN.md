...
> **Source analyzed:** React client (`client/src/**/*.jsx`) using Tailwind utilities (`bg-*`, `text-*`, `border-*`, `from-*`, `to-*`, `ring-*`) plus inline rgba overlays. This palette reflects what is actually implemented.

### Primary Brand Range (Blue)
- **Blue 900** (`{colors.primary-900}` — `oklch(37.9% 0.146 265.522)`): Header/nav deep backgrounds (`from-blue-900`, `to-blue-900`).
- **Blue 800** (`{colors.primary-800}` — `oklch(42.4% 0.199 265.638)`): Primary dark navigation and dropdown surfaces (`bg-blue-800`, `to-blue-800`).
- **Blue 700** (`{colors.primary-700}` — `oklch(48.8% 0.243 264.376)`): Hover/darker action states (`hover:bg-blue-700`, `to-blue-700`).
- **Blue 600** (`{colors.primary-600}` — `oklch(54.6% 0.245 262.881)`): Main call-to-action background (`bg-blue-600`, `from-blue-600`, `to-blue-600`).
- **Blue 500** (`{colors.primary-500}` — `oklch(62.3% 0.214 259.815)`): Bright CTA/highlight states (`bg-blue-500`, `text-blue-500`, `ring-blue-500`).
- **Blue 400 / 300 / 200 / 100 / 50** (`{colors.primary-soft-*}`): Hover text, chips, soft backgrounds (`text-blue-300`, `bg-blue-100`, `from-blue-50`, `to-blue-100`).

### Secondary Brand Range (Indigo)
- **Indigo 900–700** (`{colors.secondary-900..700}`): Deep gradient endpoints and supporting dark surfaces (`to-indigo-900`, `to-indigo-700`).
- **Indigo 600–500** (`{colors.secondary-600..500}`): Alternative CTA/hero gradients (`from-indigo-600`, `to-indigo-600`, `text-indigo-600`).
- **Indigo 300–50** (`{colors.secondary-soft-*}`): Decorative highlights and soft cards (`bg-indigo-100`, `to-indigo-100`, `from-indigo-50`).

### Neutral System (Gray + White + Black)
- **Gray 900/800/700** (`{colors.ink-strong}`): Primary text hierarchy (`text-gray-900`, `text-gray-800`, `text-gray-700`).
- **Gray 600/500/400** (`{colors.ink-muted}`): Secondary copy and iconography (`text-gray-600`, `text-gray-500`, `text-gray-400`).
- **Gray 300/200/100/50** (`{colors.surface-neutral-*}`): Inputs, panels, separators, and soft backgrounds (`bg-gray-50`, `border-gray-200`, `bg-gray-100`).
- **White** (`{colors.white}` — `#fff`) and **Black** (`{colors.black}` — `#000`): Base contrast anchors (`text-white`, translucent white overlays).

### Semantic Colors (UI Feedback)
- **Success Green** (`{colors.success-500..700}`): `green-500`, `green-600`, `green-700` for positive status and confirmations.
- **Warning Amber/Yellow** (`{colors.warning-amber-*}`, `{colors.warning-yellow-*}`): `amber-500`, `amber-400`, `yellow-500`, `yellow-400`, `yellow-300` for warning/highlight states.
- **Error Red** (`{colors.error-500..800}`): `red-500`, `red-600`, `red-700`, `red-800` for errors and destructive emphasis.
- **Support Purple** (`{colors.support-purple-*}`): `purple-400` to `purple-800` for supporting accents in info panels and highlights.

### Special Effects & Transparency
- **Translucent blue overlays**: `bg-blue-600/70`, `bg-blue-700/40`, `border-blue-600/20`, `ring-blue-300/30`.
- **Translucent white overlays**: `bg-white/10`, `bg-white/20`, `bg-white/30`, `bg-white/40`, `bg-white/50`.
- **Inline stripe overlays** (ticket/rifa visuals): `rgba(255,255,255,0.1)` and `rgba(255,255,255,0.2)` in repeating linear gradients.

### Gradient Language Used in the Project
- **Primary nav/action gradient:** `from-blue-900 to-blue-700`, `from-blue-800 to-blue-600`, `from-blue-500 to-blue-600`.
- **Secondary hero gradients:** `from-blue-600 to-indigo-700`, `from-indigo-600 to-purple-700`.
- **Soft section gradients:** `from-gray-50 to-blue-50`, `from-blue-50 to-indigo-50`, plus semantic blends (`amber`, `green`, `yellow`, `emerald`).
